...
public class MetaFieldServiceFactory : IMetaFieldServiceFactory
{
    ...
    {
        IMetaFieldService service = _serviceProvider is not null
            ? new MetaFieldService(credentials, _serviceProvider)
            : new MetaFieldService(credentials, _shopifyDomainUtility);
