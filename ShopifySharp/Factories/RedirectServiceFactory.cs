...
public class RedirectServiceFactory : IRedirectServiceFactory
{
    ...
    {
        IRedirectService service = _serviceProvider is not null
            ? new RedirectService(credentials, _serviceProvider)
            : new RedirectService(credentials, _shopifyDomainUtility);
