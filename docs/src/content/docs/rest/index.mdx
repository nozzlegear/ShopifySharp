...
description: "Shopify has deprecated their REST API, which means ShopifySharp can only support it until Shopify removes it."
sidebar:
    order: 0
    title: "Overview and deprecation"
---
Most of the "service" classes in ShopifySharp are dedicated classes for Shopify's REST API, where each class deals with a single Shopify "entity" type. For example, the [WebhookService](/rest/webhooks/) contains all the REST API operations for webhooks, and the [EventService](/rest/events/) contains all of the operations for events, and so on.
