...
Finally, all assets are tied to a specific theme, and you need that theme's id to interact with assets. You can use the [`ThemeService`](/rest/themes/) to get a list of the shop's themes, or the `ShopService` to get the currently active theme's id.
